...
interface ProductCatalogTabProps {
  ...
  analyzeGitHubFileWithAi: (file: GitHubFile, populateForm: (data: Partial<ProductForm>) => void) => void;
...
export default function ProductCatalogTab({
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const finalId = form.assetKey.trim() || "asset_custom_" + Math.random().toString(36).substring(2, 7);
    const newProductData: Omit<Product, 'image'> = {
      ...
      codeContent: form.codeContent || `// ${form.name} Blueprint source code attached successfully`
    };
                  ...
                  {repoSource === 'products.json' ? (
                    ...
                    gitFiles.map((gf, idx) => (
                      <div key={idx} className="p-3 bg-slate-900 border border-slate-850 rounded-sm flex items-center justify-between text-xs font-mono">
                        ...
                        <button
                          onClick={() => analyzeGitHubFileWithAi(gf, handleAiPopulation)}
